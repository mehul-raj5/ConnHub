...
# server needs these three environment variables to mint signed upload/download URLs.
...
> DISABLED` and `/file` sends are rejected. Text/group messaging is unaffected.
...
- **XChaCha20-Poly1305 (AEAD)**: An Authenticated Encryption with Associated Data (AEAD) cipher. Combines the ChaCha20 stream cipher with Poly1305 MAC, using a 24-byte extended nonce. Used to encrypt all message bodies end-to-end, and also to encrypt media blobs (with a fresh random 32-byte key per file) before they are uploaded to Supabase Storage.
- **SHA-256**: Used as the integrity fingerprint for media. The sender hashes the plaintext file; the recipient recomputes the hash after decrypting and drops the file if it does not match.
...
#### Media Transfers — Supabase Direct Upload (MsgType `0x04` MediaMeta)
Heavy bytes never travel through the chat server. Instead the encrypted blob is uploaded
directly to Supabase Storage and only a small **metadata envelope** is relayed:

1. **Signed-URL exchange** (client ⇄ server, plaintext control packets):
   ...
   - The server holds the Supabase `service_role` key, generates an object path scoped to
     the conversation (`<convIDhex>/<random>`), and mints time-limited signed URLs. It
     verifies the requester is a **member of that conversation** before issuing a URL.
2. **Blob transfer** (client ⇄ Supabase, direct HTTP): the sender generates a random
   32-byte key, encrypts the file with XChaCha20-Poly1305, and PUTs the ciphertext to the
   signed URL. The recipient GETs it from a signed download URL.
3. **MsgMediaMeta envelope** (fully E2E encrypted, relayed by the server like any text
   message): `[NameLen(2)] + [FileName] + [TypeLen(2)] + [FileType] + [FileSize(8)] +
   [PathLen(2)] + [ObjectPath] + [DecryptKey(32)] + [FileHash SHA-256(32)] +
   [ThumbLen(4)] + [Thumbnail JPEG]`.
- **Integrity**: the recipient recomputes SHA-256 over the decrypted bytes and silently
  drops the file if it does not match `FileHash`.
- **Backend Role**: the server only relays the tiny envelope and issues signed URLs — it
  never sees the file bytes or the decryption key.
...
2. **`handleData`**: Any standard message or media envelope simply gets mapped via `ConversationID` and distributed to all other users in that subset. The server acts entirely as a **blind relay**. Because the `FlagEncrypted` bit is set, the server ignores the body and pushes raw encrypted bytes directly into the outgoing TCP buffers of the intended recipients.
3. **`handleUploadRq` / `handleDownloadRq`** (`supabase.go`): The only endpoints that talk to Supabase. They validate conversation membership and mint short-lived signed upload/download URLs so clients can transfer encrypted blobs directly, never through this server.
...
- **Media Engine (`media.go`, `thumbnail.go`)**: On `/file`, `sendMedia` detects the MIME type, hashes and encrypts the file, requests a signed upload URL, PUTs the ciphertext to Supabase, and sends a `MsgMediaMeta` envelope. On receipt, the item is shown inline immediately (with an ANSI half-block thumbnail preview for images), then a background download fetches the blob via a signed URL, decrypts it, verifies the SHA-256, and saves it to `downloads/`.
